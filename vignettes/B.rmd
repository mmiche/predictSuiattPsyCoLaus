...
*BEWARE*: Only if the below code is executed without any changes, the simulated dataset will be superficially similar to the original dataset. Especially will the outcome be identically distributed (4002 non-cases, 48 cases).
...
## Comparison with output of original PsyCoLaus data

Original PsyCoLaus data (overview computed with rms package)
```{r echo=TRUE}
...
Simulated data (overview computed with rms package)
```{r echo=TRUE}
 #                       Model Likelihood    Discrimination    Rank Discrim.    
 #                             Ratio Test           Indexes          Indexes    
...
Frank E Harrell Jr (2021). rms: Regression Modeling Strategies. R package version 6.1-1. [https://CRAN.R-project.org/package=rms](https://CRAN.R-project.org/package=rms)